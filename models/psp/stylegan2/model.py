...
class ModulatedConv2dSimple(nn.Module):
    def __init__(
        self,
        in_channel,
        out_channel,
    ...
    def forward(self, input, style, is_stylespace=False):
        ...
        input = input.view(1, batch * in_channel, height, width)
        out = F.conv2d(input, weight, padding=self.padding, groups=batch, stride=self.stride)
        _, _, height, width = out.shape
        out = out.view(batch, self.out_channel, height, width)
...
class Generator(nn.Module):
    ...
    def forward(
        ...
        outs.append(out)
        out = self.conv1(out, styles[0] if is_stylespace else latent[:, 0], noise=noise[0], is_stylespace=is_stylespace)
        outs.append(out)

        skip = self.to_rgb1(out, to_rgb_stylespace[0] if is_stylespace else latent[:, 1], is_stylespace=is_stylespace)
        ...
        for conv1, conv2, noise1, noise2, to_rgb in zip(
            ...
            out = conv1(out, styles[i] if is_stylespace else latent[:, i], noise=noise1, is_stylespace=is_stylespace)
            outs.append(out)

            out = insert_feature(out, i + 1)
            out = conv2(out, styles[i + 1] if is_stylespace else latent[:, i + 1], noise=noise2, is_stylespace=is_stylespace)
            outs.append(out)

            skip = to_rgb(out, to_rgb_stylespace[i // 2 + 1] if is_stylespace else latent[:, i + 2], skip, is_stylespace=is_stylespace)
