...
class AttrDataset(Dataset):
    def __init__(self, images_root, attr, transform=None, attributes_root="", use_attr=True):
        ...
        for path in self.paths:
            #print("aboba")
            pic_num = int(path.split("/")[-1][:-4])
            pic_attrs = lines[pic_num + 2][:-1].split(" ")[2:]
            #print(pic_attrs[-2:])
...
class FIDDataset(Dataset):
    def __init__(self, files, transforms=None, return_names=False):
        self.files = files
    ...
    def __getitem__(self, i):
        from_path = self.files[i]
        if isinstance(from_path, str):
            img = Image.open(from_path).convert("RGB")
        else:
            img = from_path.convert("RGB")
        if self.transforms is not None:
            img = self.transforms(img)
        if self.return_names:
            return img, from_path.split('/')[-1]
        return img
...
class MetricsDataDataset(Dataset):
    def __init__(
        self, paths, inp_data, fake_data, transform=None, transform_train=None
    ):
    ...
    def __getitem__(self, index):
        ...
        from_im = self.inp_data[index]
        to_im = self.fake_data[index]
