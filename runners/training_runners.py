...
from datasets.loaders import ClassicLoader, InfiniteLoader
...
def get_random_edit():
    ...
    val = np.random.choice(directions[direction])
    return direction, val
        
def get_random_car_edit():
    direction = np.random.choice(list(car_directions.keys()))
    val = np.random.choice(car_directions[direction])
    return direction, val
...
class BaseTrainingRunner(BaseRunner):
    ...
    def _setup_datasets(self):
        ...
        self.train_dataset = ImageDataset(
            self.config.data.input_train_dir, transform_dict["transform_gt_train"]
        )

        self.test_dataset = ImageDataset(
            self.config.data.input_val_dir, transform_dict["transform_test"]
        )
        self.paths = self.test_dataset.paths

        self.special_dataset = ImageDataset(
            self.config.data.special_dir, transform_dict["transform_test"]
        )
    ...
    def _setup_dataloaders(self, batch_size):
        ...
        self.test_dataloader = ClassicLoader(
            self.test_dataset,
            batch_size=batch_size,
            shuffle=False,
            num_workers=self.config.model.workers,
        )
        self.special_dataloader = ClassicLoader(
    ...
    def _setup_loss(self):
        ...
        self.loss_builder = LossBuilder(enc_losses_dict, 
                                        disc_losses_dict, 
                                        self.device)
        ...
        for self.global_step in range(self.start_step, self.config.train.steps + 1):
            ...
            if self.global_step % self.config.train.val_step == 0 :
                with Timer(iter_info, "iter_val"):
                    ...
                    self.logger.save_validation_logs(orig_pics,
                                                    method_pics, 
                                                    captions, 
    ...
    def forward(self, x):
        output = self.method.forward(x, return_latents=True)
        return output
    ...
    def get_save_dict(self):
        save_dict = {"state_dict": self.method.state_dict(),
    ...
    def _run_on_batch(self, inputs):
        if self.masked:
            inputs, mask = inputs
            result_batch = self.method(inputs, mask=mask)
        else:
...
class FSEInverterTrainingRunner(BaseTrainingRunner):
    def forward(self, x):
        y_hat_inv, w_inv, fused_feat, w_feat = self.method(x,
                                                   return_latents=True,
                                                   n_iter=self.global_step)
                                                
        y_hat_inv_w, _ = self.method.decoder([w_inv],
    ...
    def forward(self, x):
        ...
        with torch.no_grad():
            ...
            if isinstance(edited_w_e4e, tuple):
                # stylespace case
                y_E, fy_e4e = self.method.decoder(edited_w_e4e, 
                                                  is_stylespace=True, 
                                                  input_is_latent=True,
                                                  randomize_noise=False,
                                                  return_features=True)
            ...
            _, x_E_w_feats = self.method.decoder([w_x_E],
                                                input_is_latent=True,
                                                return_features=True,
                                                is_stylespace=False,
                                                randomize_noise=False,
        ...
        y_hat_edit, _ = self.method.decoder(w_x_E_edited,
                                            input_is_latent=True,
                                            features_in=x_E_edited_feats,
                                            feature_scale=1.0,
        ...
        if use_adv_loss:
            if x_E_256.size(0) > x_resh.size(0):
                ...
                output["encoder"]["fake_preds"] = self.method.discriminator(torch.cat([y_hat_inv, y_hat_edit[bs:]], dim=0), None)
            else:
