...
def apply_mask(x, y, start_idx, container):
    bs = x.size(0)
    if x.dim() == 3:
        bs = 1
    mask = torch.stack(container[start_idx:start_idx + bs], dim=0)
    assert mask.dim() == 4, mask.shape
    res = x.size(-1)
    mask = F.interpolate(mask, size=(res, res), mode="bilinear", align_corners=False).float().to(x.device)
    res = x * (1 - mask) + y * mask
    return res
...
@metrics_registry.add_to_registry(name="idd")
class IDMetric:
    def __init__(self, batch_size=4, n_workers=4, masked=False):
        self.loss_func = IDLoss()
        self.batch_size = batch_size
        self.n_workers = n_workers
